'use client';
import{useEffect,useMemo,useState}from'react';import Link from'next/link';import {useRouter,useSearchParams} from'next/navigation';import type{IncidentRecord,Node}from'@/lib/model';

export default function IncidentsConsole(){
  const router=useRouter();
  const[incidents,setIncidents]=useState<IncidentRecord[]|null>(null);
  const[nodes,setNodes]=useState<Node[]>([]);
  const[nodeFilter,setNodeFilter]=useState(''),[tagFilter,setTagFilter]=useState(''),[stateFilter,setStateFilter]=useState('');
  ...
  const allStates=useMemo(()=>[...new Set((incidents??[]).map(i=>i.state))].sort(),[incidents]);
  ...
  return <main><section className="hero"><div className="eyebrow">Timeline</div><h1>{showAll?'All incidents':'Active incidents'}</h1>{showAll&&<Link href="/incidents" className="muted small">Show active only</Link>}</section>
    ...
    <div className="filter-row">
      ...
      <select value={stateFilter} onChange={e=>setStateFilter(e.target.value)}>
        ...
        {allStates.map(s=><option value={s} key={s}>{s}</option>)}
      </select>
    </div>
