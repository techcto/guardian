...
export default function BillingConsole(){
 ...
 return <>{error&&<div className="notice"><strong>Billing unavailable</strong><span>{error}</span></div>}<section className="grid">{products.map(p=><article className="card price-card" key={p.id}><div className="eyebrow">{p.name}</div><div className="price"><strong>${p.monthlyPrice}</strong><span>/month</span></div><p className="muted">{p.description}</p><p>Up to {p.serverLimit} servers</p><button className="button primary" disabled={!p.available} onClick={()=>checkout(p.id)}>{p.available?'Choose plan':'Configure Stripe price'}</button></article>)}</section></>
}
