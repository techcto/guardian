...
export type Node={tenantId:string;serverId:string;agentId:string;displayName:string;platform?:string;clusterId?:string;status:NodeStatus;createdAt:string;lastHeartbeat:string};
...
export type Settings={
  tenantId:string;
  detection:{warningPercent:number;criticalPercent:number;emergencyPercent:number;trafficMultiplier:number;distributedCrawlerCorrelation:boolean;applicationFailureDetection:boolean};
...
export function defaultSettings(tenantId:string):Settings{return{
  tenantId,
  detection:{warningPercent:2,criticalPercent:10,emergencyPercent:30,trafficMultiplier:5,distributedCrawlerCorrelation:true,applicationFailureDetection:true},
